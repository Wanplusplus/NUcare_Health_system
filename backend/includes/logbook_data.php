...
function nucareLogbookNormalizeKey(string $value): string
{
 $value = strtolower(trim($value));
 return preg_replace('/[^a-z0-9]+/', '', $value) ?? '';
}

function nucareLogbookBucketPersonCategory(array $row): string
{
...
function nucareLogbookClassifyConsultationRow(array $row): ?string
{
...
function nucareLogbookAddBucket(array &$matrix, string $rowName, string $bucket, int $qty = 1): void
{
...
function nucareLogbookDefinitions(): array
{
 return [
 'serviceRows' => [
 'PE',
...
function nucareBuildLogbookData(PDO $pdo, int $year, int $month): array
{
 if ($month < 1 || $month > 12) {
 $month = (int)date('n');
 }
 ...
 $defs = nucareLogbookDefinitions();
 $serviceRows = $defs['serviceRows'];
 $medicineRows = $defs['medicineRows'];
 $supplyRows = $defs['supplyRows'];
 ...
 $medicineLookup[nucareLogbookNormalizeKey($label)] = $label;
 }
 $medicineLookup[nucareLogbookNormalizeKey('Diphenhydramine Amp')] = 'Diphenhydramine';
 $supplyLookup = [];
 foreach ($supplyRows as $label) {
 $supplyLookup[nucareLogbookNormalizeKey($label)] = $label;
 }
 ...
 $consultationTotal++;
 $bucket = nucareLogbookBucketPersonCategory($row);
 $serviceRow = nucareLogbookClassifyConsultationRow($row);

 if ($serviceRow !== null && $serviceRow !== '__DENTAL__' && $serviceRow !== '__DAILY__') {
 nucareLogbookAddBucket($serviceMatrix, $serviceRow, $bucket, 1);
 continue;
 }
 ...
 nucareLogbookAddBucket($serviceMatrix, 'PE', $bucket, 1);
 continue;
 }
 ...
 $normalizedMedicine = nucareLogbookNormalizeKey($medicineName);
 $normalizedGeneric = nucareLogbookNormalizeKey($genericName);
 ...
 'dentalMatrix' => $dentalMatrix,
 'dailyMatrix' => $dailyMatrix,
 'medicineTotals' => $medicineTotals,
