...
| Most Active Users | ⚠️ Partially Supported | `audit_logs GROUP BY UserID` — only counts non-suppressed actions |
...
**Overall Status: ⚠️ PARTIALLY SUPPORTED** — Login/logout tracked; failed activities suppressed; no session tracking.
...
- Summary PDF: ✅
- Detailed PDF: ✅
- Recommended Layout: Role list table + Matrix view (Role × Module × Permission) + User count per role
...
| Activity history by role | ⚠️ Partially Supported | Must JOIN `users → user_roles → roles` — user may have multiple roles |
...
**Overall Status: ⚠️ PARTIALLY SUPPORTED** — Core audit data present; suppressed actions create gaps.
...
| Login counts per user | ⚠️ Partially Supported | Only successful logins can be counted |
| Login counts by date range | ⚠️ Partially Supported | Only successful logins within date range |
...
| Preset | Supported | Timestamp Column |
|--------|-----------|-----------------|
| Today | ⚠️ | Successful only (`audit_logs.ActionTimestamp`) |
| This Week | ⚠️ | Successful only |
| This Month | ⚠️ | Successful only |
| Custom Range | ⚠️ | Successful only |
...
**Overall Status: ⚠️ PARTIALLY SUPPORTED** — Successful logins only; failed logins require schema/code fix.
...
| Recently blocked accounts | ⚠️ Partially Supported | Can filter `audit_logs WHERE Action = 'Deactivated user account'` — but no `blocked_at` timestamp on users table |
...
**Overall Status: ⚠️ PARTIALLY SUPPORTED** — Current status fully supported; deactivation history partial.
...
| 2 | Account Activity | ⚠️ Partially Supported | ⚠️ Suppressed actions | ✅ Full | ✅ |
| 3 | Role & Permission | ✅ Fully Supported | ✅ All | N/A | ✅ |
| 4 | Audit Log | ⚠️ Partially Supported | ⚠️ Suppressed actions | ✅ Full | ✅ |
| 5 | Login Report | ⚠️ Partially Supported | ❌ Failed logins suppressed | ⚠️ Partial | ✅ |
| 6 | Account Status | ⚠️ Partially Supported | ⚠️ No blocked_at timestamp | ✅ Full | ✅ |
...
| **Audit Log Report** | ⚠️ Partially Supported | Medium | Suppressed actions create gaps in activity history |
| **Account Activity Report** | ⚠️ Partially Supported | Medium | Failed activities not tracked; no session data |
| **Login Report** | ⚠️ Partially Supported | Medium | Successful logins only; failed logins require fix |
...
| Role & Permission | Matrix table (Role × Module × Permission) | Role distribution summary with user counts |
